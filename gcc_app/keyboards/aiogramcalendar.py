...
def create_calendar(year=datetime.now().year, month=datetime.now().month):
    ...
    inline_kb.insert(
        InlineKeyboardButton(f"{str(year)}", callback_data=ignore_callback)
    )
    inline_kb.insert(
        InlineKeyboardButton(
    ...
    inline_kb.insert(
        InlineKeyboardButton(
            f"{calendar.month_name[month]}", callback_data=ignore_callback
        )
    )
    inline_kb.insert(
        InlineKeyboardButton(
...
async def process_calendar_selection(query, data):
    ...
    # user navigates to previous year, editing message with new calendar
        ...
        await query.message.edit_reply_markup(
            create_calendar(int(prev_date.year), int(prev_date.month))
        )
    # user navigates to next year, editing message with new calendar
    elif data["act"] == "NEXT-YEAR":
        next_date = temp_date + timedelta(days=365)
        await query.message.edit_reply_markup(
            create_calendar(int(next_date.year), int(next_date.month))
        )
