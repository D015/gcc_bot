...
from gcc_app.constants import (KEY_UNFINISHED_EVENT_CREATION, CODE)
...
async def process_code_link(message: types.Message):
    ...
    if text.startswith('http') and '.' in text and '//' in text:
        ...
        unfinished_event_creation.update({CODE: text})
        redis_set(redis_name, unfinished_event_creation)
        state = dp.current_state(user=message.from_user.id)
        await state.set_state(EventCreationStates.all()[4])
